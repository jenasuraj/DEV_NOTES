...
8. [Dynamic Data Fetching](#dynamic-data-fetching)
   - [Fetch on Mount](#fetch-on-mount)
   - [Fetch on Scroll (Triggered Fetching)](#fetch-on-scroll-triggered-fetching)
   - [Pagination and Infinite Scroll](#pagination-and-infinite-scroll)
   - [Caching Fetches in Next.js](#caching-fetches-in-nextjs)
...
**2. Move heavy work off the main thread using Web Workers**

Web Workers run in a separate thread entirely — they cannot touch the DOM, but they can do heavy computation without blocking the UI:
...
worker.onmessage = (e) => {
  setResult(e.data); // update UI with the result
};
```
...
So far we've covered lazy loading components — but what about the data those components display? Fetching data efficiently is equally important for performance.

### Fetch on Mount

The basic pattern — fetch data when a component mounts. Fine for small, non-critical data.
...
### Fetch on Scroll (Triggered Fetching)

Combine IntersectionObserver with data fetching — don't fetch data until the section is actually visible. This is powerful for below-the-fold sections that load from an API.
...
type Testimonial = { id: number; name: string; quote: string };

export default function TestimonialsSection() {
  const [testimonials, setTestimonials] = useState<Testimonial[]>([]);
  const [loaded, setLoaded] = useState(false);

  // When this section enters the viewport, fetch the data
  const { ref } = useInView({
    triggerOnce: true,
    rootMargin: '300px',
    onChange: (inView) => {
  ...
  return (
    <section ref={ref} className="py-20">
      ...
      {!loaded ? (
        ...
        <div className="grid grid-cols-3 gap-6">
          {testimonials.map(t => (
            <div key={t.id} className="p-6 border rounded-xl">
              <p>{t.quote}</p>
              <span>{t.name}</span>
            </div>
          ))}
...
When you have a large list, never fetch all items at once. Fetch one page at a time, and load the next page when the user reaches the bottom.
...
export default function InfinitePostList() {
  ...
  const loadMore = useCallback(async () => {
    ...
    if (newPosts.length < 10) setHasMore(false); // no more pages
    setPosts(prev => [...prev, ...newPosts]);
    setPage(prev => prev + 1);
    setLoading(false);
  }, [page, loading, hasMore]);

  // This sentinel div sits at the bottom of the list.
  ...
  return (
    <div>
      ...
      {!hasMore && <div className="text-center py-4 text-gray-400">You've reached the end</div>}
    </div>
  );
}
```
...
### Caching Fetches in Next.js

In Next.js App Router, the built-in `fetch` is extended with caching options. Use these in Server Components to avoid redundant network requests:

```ts
// Cached indefinitely — perfect for data that never changes
const res = await fetch('/api/config', {
  cache: 'force-cache'
});

// Never cached — always fresh (same as no-store)
const res = await fetch('/api/live-scores', {
  cache: 'no-store'
});

// ISR style — cached for 1 hour, then revalidated in background
const res = await fetch('/api/blogs', {
  next: { revalidate: 3600 }
});

// Tag-based revalidation — revalidate on demand via revalidateTag()
...
**Next.js automatically deduplicates fetch calls** within the same render cycle. If `generateMetadata` and your Page component both call `fetch('/api/blogs/my-post')` with the same options, only one real network request is made. The second call gets the cached result from the first.
